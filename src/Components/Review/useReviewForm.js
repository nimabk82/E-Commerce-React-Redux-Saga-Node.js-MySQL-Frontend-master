...
export default function useReviewForm() {
  ...
  const save = (name, value) => {
    let newValue =
      name in ["fit", "overall", "length", "comfort"] ? parseInt(value) : value;
  ...
  const isValid = () => {
    ...
    newErrors.title =
      ...
      !/^[A-Za-z]/i.test(state.title)
        ? true
        : false;
    newErrors.name =
      state.name === "" ||
      state.name.length > 50 ||
      !/^[A-Za-z]/i.test(state.name)
        ? true
        : false;
    newErrors.email =
      state.email === "" ||
      !/^[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,4}$/i.test(state.email)
        ...
        : false;
    newErrors.body =
      state.body === "" || !/^[A-Za-z]/i.test(state.body) ? true : false;
    return newErrors;
  };
