...
export default function RightSideData({ id }) {
  ...
  return (
    <div className="description">
      ...
      {product.salesRate === 1 && (
        <p className="non-sales-price">
          ${price} {currency}
        </p>
      )}
      {product.salesRate < 1 && (
        <p className="sales-price">
          SALE ${price * product.salesRate} {currency} (
          {100 - product.salesRate * 100} % off)
        </p>
      )}
      ...
      <SizeTab
        onChange={(size) => handleChangeSize(size)}
        sizeDisplay={sizeAvailability}
      />
      <Button variant="outlined" id="add-to-cart-btn" onClick={handleAddToCart}>
        <b>Add to Cart</b>{" "}
      </Button>
