  ...
  switch (action.type) {
    ...
    case REMOVE_ITEM:
      ...
      let itemToRemove = state.addedItems.find(
        (item) => action.id === item.id && action.size === item.size
      );
      if (!itemToRemove) {
        return state;
      }
    ...
    case SUB_QUANTITY:
      ...
      let itemToSub = newState.addedItems.find(
        (item) => action.id === item.id && action.size === item.size
      );
      ...
      return newState;
    case ADD_QUANTITY:
      let itemToAdd = newState.addedItems.find(
        (item) => action.id === item.id && action.size === item.size
      );
      if (itemToAdd) {
