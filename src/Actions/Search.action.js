...
export function findBySingleColor(color) {
  ...
  let result = categoryData.filter(
    (product) => product.color === colorLowerCase
  );
  return result;
}
...
export function findBySingleSize(size) {
  ...
  let result = stockData.map((product) => {
    let availbleProduct = product.stock.find(
      (item) => item.size === upperCaseSize && item.quantity > 0
    );
...
export function showStockStatus(id) {
  ...
  let availableStock = item.stock.map((each) =>
    each.quantity > 0 ? true : false
  );
...
export function findByColor(colorList) {
  ...
  lowerCaseColorList.forEach((color) => {
    midresult = categoryData.filter((product) => product.color === color);
...
export function findByName(name) {
  ...
  let resultList = categoryData.filter(
    (product) => product.name === nameLowerCase
  );
  return resultList;
}
...
export function findByHeel(heelList) {
  ...
  heelList.forEach((heel) => {
    midresult = categoryData.filter(
      (product) => product.heel.toString() === heel
    );
    // midresult is correct but this is wrong
    resultList = resultList.concat(midresult);
  });
...
export function findByColorAndHeel(colorList, heelList) {
  ...
  const resultList = categoryData.filter(
    (product) => product.color in lowerCaseColorList && product.heel in heelList
  );
  return resultList;
}
